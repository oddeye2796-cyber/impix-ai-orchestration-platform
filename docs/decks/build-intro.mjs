...
import { C, F, W, H, M, COL, page, card, chip, title, sub, foot, stat, SHOTS, useLocale, tr, reportMissing } from './theme.mjs';

// One generator per deck, one copy file per language. DECK_LOCALE picks which.
const LOCALE = process.env.DECK_LOCALE || 'ko';
const { copy, introFile } = await import(`./copy/${LOCALE}.mjs`);
useLocale(LOCALE, copy);

const pres = new pptxgen();
pres.layout = 'LAYOUT_WIDE';
pres.author = 'IMPIX AI';
pres.title = tr('IMPIX AI Orchestration Platform 소개서');

let n = 0;
const FOOT = tr('IMPIX AI Orchestration Platform · 플랫폼 소개서');
...
/* ───────────────────────── 1. 표지 ───────────────────────── */
{
  ...
  s.addText(tr('日本語 · English · 한국어'), { x: M, y: 6.15, w: 3.15, h: 0.42, margin: 0, align: 'center', valign: 'middle', fontFace: F.ko, fontSize: 11, bold: true, color: C.accent, isTextBox: true });
  s.addText(tr('플랫폼 소개서'), { x: W - M - 3, y: 6.2, w: 3, h: 0.32, margin: 0, align: 'right', fontFace: F.ko, fontSize: 12, color: C.dim, isTextBox: true });
  s.addNotes(tr('표지. 제품의 다크 테마 팔레트를 그대로 사용해 화면 캡처와 색이 이어지도록 했습니다.'));
}

/* ───────────────────────── 2. 한 장 요약 ───────────────────────── */
{
  const s = New(); n = 2;
  title(s, tr('한 장으로 보는 IMPIX AI'), { kicker: 'AT A GLANCE' });
  sub(s, tr('개별 AI 모델을 나열한 대시보드가 아니라, 서로 다른 목표를 가진 에이전트들이 협의하고 사람이 최종 승인하는 의사결정 구조입니다.'), 1.42);

  const items = [
    ['6', tr('AI 에이전트'), tr('품질 · 예지보전 · 생산\n에너지 · 안전 · 물류'), C.accent],
    ['32', tr('기능 모듈'), tr('8개 카테고리에 걸친\n운영 화면 일체'), C.cyan],
    ['4', tr('체험 시나리오'), tr('실제 공장에서 발생하는\n상황을 단계별로 재현'), C.warn],
    ['98.2%', tr('의사결정 신뢰도'), tr('에이전트 합의(Ensemble\nVoting) 기반 최종 판단'), C.violet],
  ];
  ...
  s.addText(tr('핵심 명제'), { x: M + 0.42, y: 5.0, w: 2, h: 0.3, margin: 0, fontFace: F.ko, fontSize: 11, bold: true, color: C.accent, charSpacing: 1, isTextBox: true });
  s.addText(tr('“생산을 늘려라”와 “안전을 지켜라”는 늘 충돌합니다. IMPIX AI는 이 충돌을 숨기지 않고 화면에 드러낸 뒤,\n정책 가드레일과 에이전트 합의를 거쳐 근거가 남는 하나의 결론으로 좁힙니다.'), {
...
{
  const s = New(); n = 3;
  title(s, tr('현장의 세 가지 단절'), { kicker: 'PROBLEM' });
  sub(s, tr('스마트팩토리에 투자했는데도 판단이 빨라지지 않는 이유는 대개 모델의 성능이 아니라 구조에 있습니다.'), 1.42);

  const rows = [
    ['01', tr('사일로'), tr('품질·설비·에너지·안전 시스템이 각자의 화면에서 각자의 경고를 냅니다. 어떤 경고를 먼저 처리해야 하는지는 아무도 알려주지 않습니다.'), C.danger],
    ['02', tr('대응 지연'), tr('이상 징후를 발견해도 담당자를 찾고 근거를 모으는 데 시간이 흘러갑니다. 설비가 멈춘 뒤에야 회의가 시작됩니다.'), C.warn],
    ['03', tr('블랙박스'), tr('AI가 값을 하나 던져주지만 왜 그 값인지 알 수 없습니다. 근거를 설명할 수 없는 제안은 현장에서 채택되지 않습니다.'), C.violet],
  ];
  const rh = 1.22;
  rows.forEach(([num, head, body, col], i) => {
    const y = 2.15 + i * (rh + 0.24);
  ...
  s.addText(tr('→  세 가지 모두 “모델을 하나 더 붙이는 일”로는 풀리지 않습니다. 필요한 것은 조율 계층입니다.'), {
    x: M, y: 6.45, w: COL, h: 0.36, margin: 0, fontFace: F.ko, fontSize: 13, bold: true, color: C.accent, isTextBox: true });
  foot(s, n, FOOT);
}
...
{
  const s = New(); n = 4;
  title(s, tr('6개의 도메인 에이전트, 1개의 슈퍼바이저'), { kicker: 'SOLUTION' });
  sub(s, tr('각 에이전트는 자기 도메인의 목표만 봅니다. 서로 부딪히는 지점을 정리하는 일은 슈퍼바이저의 몫입니다.'), 1.42);

  const ag = [
    ['Quality Agent', tr('품질'), tr('YOLOv8 · PatchCore 비전 검사\n불량률 임계치 5% / 8% 관리'), C.accent],
    ['PM Agent', tr('설비 예지보전'), tr('GBR 잔여수명(RUL) 예측\n72시간 전 사전 워크오더'), C.cyan],
    ['Line Monitor Agent', tr('생산 최적화'), tr('OEE · 택트타임 · 병목 진단\n가동률 95% 상시 유지'), C.violet],
    ['Energy Agent', tr('에너지'), tr('15분 단위 피크 부하 제어\n계약 전력 90% 이내 관리'), C.warn],
    ['Safety Agent', tr('안전'), tr('CCTV 화재 감지 · AMR 충돌 방지\n비상 정지 전파 100ms'), C.danger],
    ['Logistics Agent', tr('물류'), tr('AMR 배차 · 출하 SLA 관리\n러시 오더 대응 조율'), '38BDF8'],
  ];
  ...
  s.addText(tr('전사 목표(Active Goal)를 각 에이전트의 하위 과업으로 분해하고, 제안이 충돌하면 정책 가드레일 순서에 따라 중재합니다.\nL1 Safety Layer가 걸리면 생산 목표를 블로킹합니다 — 안전이 생산을 이기는 규칙이 코드에 있습니다.'), {
...
{
  const s = New(); n = 5;
  title(s, tr('데이터에서 승인까지, 하나의 경로'), { kicker: 'ARCHITECTURE' });
  sub(s, tr('모든 판단은 같은 파이프라인을 지나며, 각 단계가 화면에 그대로 노출됩니다.'), 1.42);

  const steps = [
    ['01', tr('수집'), tr('Kafka 스트림\nTimescaleDB 시계열'), C.muted],
    ['02', tr('추론'), tr('에이전트별 전용 모델\n온톨로지 규칙 평가'), C.cyan],
    ['03', tr('조율'), tr('Supervisor 중재\n합의 · 정책 가드레일'), C.accent],
    ['04', tr('승인'), tr('Human Gate\n사람이 최종 결정'), C.warn],
    ['05', tr('실행 · 기록'), tr('제어 명령 반영\n감사 추적 로그 적재'), C.violet],
  ];
  ...
  steps.forEach(([num, head, body, col], i) => {
    ...
    s.addText(body, { x: x + 0.26, y: 3.42, w: cw - 0.52, h: 0.78, margin: 0, lineSpacing: 15, fontFace: F.ko, fontSize: 10.5, color: C.muted, isTextBox: true });
  ...
  s.addText(tr('세 단계가 이 플랫폼을 다른 대시보드와 구분합니다'), { x: M + 0.42, y: 4.72, w: COL - 0.84, h: 0.34, margin: 0, fontFace: F.ko, fontSize: 15, bold: true, color: C.text, isTextBox: true });
  const hi = [
    [tr('03 조율'), tr('에이전트끼리 반대 의견을 내고 근거를 주고받는 과정 자체를 시각화합니다.'), C.accent],
    [tr('04 승인'), tr('Level 2 이상 제어는 사람이 승인해야만 실행됩니다. 자동화의 범위를 사람이 정합니다.'), C.warn],
    [tr('05 기록'), tr('누가 무엇을 왜 승인했는지 남습니다. 감사와 사후 분석이 가능해집니다.'), C.violet],
  ];
...
{
  const s = New(); n = 6;
  title(s, tr('에이전트가 협의하는 과정을 보여줍니다'), { kicker: 'DIFFERENTIATOR 01' });

  // 2050 x 1790 crop -> 1.145 aspect
  const iw6 = 5.65, ih6 = iw6 / 1.145;
  s.addImage({ path: `${SHOTS.path}/20-orchestration-crop.png`, x: M, y: 1.5, w: iw6, h: ih6 });
  s.addShape('roundRect', { x: M, y: 1.5, w: iw6, h: ih6, rectRadius: 0.02, fill: { color: C.bg, transparency: 100 }, line: { color: C.border, width: 1 } });
  s.addText(tr('AI 오케스트레이션 센터 — 에이전트 간 의사결정 조율 시각화'), { x: M, y: 1.5 + ih6 + 0.12, w: iw6, h: 0.3, margin: 0, fontFace: F.ko, fontSize: 10, color: C.muted, isTextBox: true });

  const bx = M + 5.65 + 0.45, bw = COL - 5.65 - 0.45;
  const pts = [
    [tr('제안'), tr('물류 에이전트가 “패키징 1라인 속도 20% 상향”을 제안합니다.')],
    [tr('반박'), tr('품질 에이전트가 열접합 스크래치 불량 상승을 근거로 반대합니다.')],
    [tr('보완'), tr('예지보전 에이전트가 냉각 파라미터 미세 조정으로 발열을 억제할 수 있다고 제시합니다.')],
    [tr('합의'), tr('슈퍼바이저가 “속도 +5% + ESS 120kW 방전 + AMR 완충 버퍼”로 결론을 냅니다.')],
  ];
  let y = 1.5;
  pts.forEach(([h, b], i) => {
  ...
  s.addText(tr('충돌을 감춘 자동화는 신뢰를 얻지 못합니다.\n반대 의견이 보이기 때문에 결론을 받아들일 수 있습니다.'), {
...
{
  const s = New(); n = 7;
  title(s, tr('자동화의 한계선을 사람이 정합니다'), { kicker: 'DIFFERENTIATOR 02' });
  sub(s, tr('Level 2 이상의 제어는 승인 대기 큐에 쌓이고, 승인 전까지 어떤 명령도 설비로 나가지 않습니다.'), 1.42);

  const bw2 = 5.05;
  const lv = [
    ['L0', tr('관찰'), tr('수집과 표시만 합니다. 제어 없음.'), C.dim],
    ['L1', tr('권고'), tr('추천을 카드로 띄웁니다. 실행은 사람이.'), C.cyan],
    ['L2', tr('승인 후 자율'), tr('승인 대기 큐를 거쳐야 실행됩니다.'), C.warn],
    ['L3', tr('가드레일 내 자율'), tr('정책 범위 안에서만 스스로 실행합니다.'), C.accent],
  ];
  ...
  s.addImage({ path: `${SHOTS.path}/12-approval-main.png`, x: ix, y: 2.15, w: iw7, h: ih7 });
...
{
  const s = New(); n = 8;
  title(s, tr('8개 카테고리 · 32개 모듈'), { kicker: 'FUNCTIONAL MAP' });
  sub(s, tr('모든 모듈이 같은 구조를 따릅니다 — 사용 시나리오 / 내부 로직 / 목표·가드레일 / 산출물.'), 1.42);

  const cats = [
    [tr('시스템 개요'), 1, [tr('통합 대시보드')], C.text],
    [tr('AI 품질'), 5, [tr('실시간 불량률'), tr('비전검사 뷰어'), tr('불량 유형 분석'), tr('온도-불량 상관'), tr('품질 이력 검색')], C.accent],
    [tr('AI 설비예지보전'), 5, [tr('설비 상태 종합'), tr('진동/온도 추세'), tr('RUL 잔여수명'), tr('보전 작업 오더'), tr('보전 이력 조회')], C.cyan],
    [tr('AI 생산 최적화'), 5, [tr('라인별 가동 현황'), tr('OEE 대시보드'), tr('택트타임 분석'), tr('배치별 생산 이력'), tr('계획 대비 실적')], C.violet],
    [tr('AI 에너지'), 3, [tr('전력 사용 현황'), tr('에너지 비용 분석'), tr('피크 부하 관리')], C.warn],
    [tr('AI 안전'), 3, [tr('CCTV 화재 감시'), tr('AMR 충돌 방지'), tr('긴급 정지·알람')], C.danger],
    [tr('AI 슈퍼바이저'), 5, [tr('AI 챗봇'), tr('AI 추천 액션'), tr('Agent 실행 이력'), tr('승인 대기 큐'), tr('슈퍼바이저 센터')], '38BDF8'],
    [tr('시스템 관리'), 5, [tr('온톨로지 관리'), tr('모델 레지스트리'), tr('사용자/권한'), tr('시스템 모니터링'), tr('알림 설정')], C.muted],
  ];
...
{
  const s = New(); n = 9;
  title(s, tr('네 가지 체험 시나리오'), { kicker: 'SCENARIOS' });
  sub(s, tr('각 시나리오는 4단계로 진행되며, 관람객이 직접 승인 버튼을 눌러 결과를 바꿔볼 수 있습니다.'), 1.42);

  const sc = [
    ['A', tr('전사 생산 극대화 & 통합 OEE'), tr('생산 극대화 목표 아래 품질·예지보전·에너지·안전·물류 전 에이전트가 실시간으로 협업합니다.'), tr('실시간 진단 → RUL 예측 → 지휘 타워 → 라이브 조율'), C.accent],
    ['B', tr('돌발 고장 및 안전 예방 가드레일'), tr('미세 가스 감지에서 시작해 공조 제어, 비전 감시, L1 Safety 정책까지 연쇄 대응합니다.'), tr('초동 조치 → CCTV 감시 → 안전 우선 정책 → 배기 조율'), C.danger],
    ['C', tr('피크 전력 한계 돌파 억제'), tr('한낮 냉방 부하로 계약 전력 초과 30분 전, ESS와 유휴 AMR 배터리까지 동원합니다.'), tr('피크 감시 → 부하 정비 → 탄소 정책 → ESS 셰이빙'), C.warn],
    ['D', tr('긴급 러시 오더 & 불량 보증'), tr('납기와 품질이 정면으로 충돌하는 상황에서 속도 120% 가속과 불량률 제로를 동시에 노립니다.'), tr('초가속 모드 → 비전 검사 → SLA 가드 → 스크래치 방어'), C.violet],
  ];
...
{
  const s = New(); n = 10;
  title(s, tr('모듈마다 숫자로 된 목표가 붙어 있습니다'), { kicker: 'TARGETS & GUARDRAILS' });
  sub(s, tr('“AI를 도입했다”가 아니라 “무엇을 몇까지”가 각 화면에 명시되어 있습니다. 아래는 대표 지표입니다.'), 1.42);

  s.addChart('bar', [{
    name: tr('목표치 (%)'),
    labels: [tr('OEE 종합효율'), tr('설비 가동률'), tr('화재 감지 정확도'), tr('자연어 응답 정확도'), tr('AI 추천 승인율'), tr('시스템 가동률')],
    values: [85, 95, 99, 95, 90, 99.9],
  }], {
    x: M, y: 2.15, w: 7.4, h: 4.1,
    barDir: 'bar', barGapWidthPct: 45,
    chartColors: [C.accent],
    showTitle: true, title: tr('달성 목표 지표 (%)'), titleColor: C.text, titleFontFace: F.ko, titleFontSize: 13,
  ...
  const lat = [
    ['100 ms', tr('비상 정지 명령 전파')],
    ['50 ms', tr('AMR 충돌 센서 처리')],
    [tr('2 초'), tr('화재 감지 후 알림 전파')],
    [tr('72 시간'), tr('고장 사전 예측 리드타임')],
    [tr('10 분'), tr('승인 프로세스 평균 처리')],
  ];
  card(s, { x: rx, y: 2.15, w: rw, h: 4.1 });
  s.addText(tr('시간 제약'), { x: rx + 0.3, y: 2.36, w: rw - 0.6, h: 0.3, margin: 0, fontFace: F.ko, fontSize: 13, bold: true, color: C.text, isTextBox: true });
  lat.forEach(([v, l], i) => {
    const y = 2.82 + i * 0.66;
...
{
  const s = New(); n = 11;
  title(s, tr('설명 없이도 굴러가는 부스 데모'), { kicker: 'EXHIBITION' });
  sub(s, tr('관람객이 혼자 들어와도 3초 안에 무엇을 눌러야 할지 알 수 있도록 설계했습니다.'), 1.42);

  s.addImage({ path: `${SHOTS.path}/01-landing.png`, x: M, y: 2.05, w: 7.3, h: 4.47 });
  ...
  const flow = [
    [tr('진입'), tr('3개의 경로 — 시나리오 선택 / 바로 시작 / 관리자 모드. 관람객은 앞의 둘만 씁니다.')],
    [tr('가이드'), tr('화면 좌측에 체험 가이드가 붙어 단계·미션·체크리스트를 안내합니다.')],
    [tr('조작'), tr('“이 버튼을 눌러보세요” 수준까지 지시합니다. 안내자가 없어도 진행됩니다.')],
    [tr('완료'), tr('4단계를 마치면 미션 달성 배너가 뜨고 처음으로 돌아갑니다.')],
  ];
  let y = 2.05;
  flow.forEach(([h, b], i) => {
  ...
  s.addText(tr('특정 전시회 이름이 화면에 남지 않습니다.\n어느 전시회에서도 그대로 사용할 수 있습니다.'), {
...
{
  const s = New(); n = 12;
  title(s, tr('일본어 · 영어 · 한국어, 다크 / 라이트'), { kicker: 'LOCALIZATION' });
  sub(s, tr('1,449개 문자열을 카탈로그로 관리합니다. 언어를 바꿔도 진행 중인 투어와 대화 이력은 유지됩니다.'), 1.42);

  const iw = (COL - 0.36) / 2;
  s.addImage({ path: `${SHOTS.path}/30-dashboard-ja.png`, x: M, y: 2.1, w: iw, h: 3.19 });
  s.addShape('roundRect', { x: M, y: 2.1, w: iw, h: 3.19, rectRadius: 0.02, fill: { color: C.bg, transparency: 100 }, line: { color: C.border, width: 1 } });
  s.addText(tr('日本語 · 다크 테마 (기본값)'), { x: M, y: 5.36, w: iw, h: 0.3, margin: 0, fontFace: F.ko, fontSize: 11, bold: true, color: C.accent, isTextBox: true });

  s.addImage({ path: `${SHOTS.path}/31-dashboard-en-light.png`, x: M + iw + 0.36, y: 2.1, w: iw, h: 3.19 });
  ...
  s.addText(tr('English · 라이트 테마'), { x: M + iw + 0.36, y: 5.36, w: iw, h: 0.3, margin: 0, fontFace: F.ko, fontSize: 11, bold: true, color: C.cyan, isTextBox: true });

  const facts = [
    [tr('자동 감지'), tr('브라우저 언어를 읽어 첫 화면부터 맞춥니다.')],
    [tr('상태 보존'), tr('언어를 바꿔도 화면이 다시 로드되지 않습니다.')],
    [tr('AI 응답'), tr('슈퍼바이저 챗봇도 선택한 언어로 답합니다.')],
  ];
...
{
  const s = New(); n = 13;
  title(s, tr('기술 구성과 운영'), { kicker: 'TECHNOLOGY' });
  sub(s, tr('화면은 정적 사이트로 배포되고, AI 키는 브라우저에 실리지 않습니다.'), 1.42);

  const groups = [
    [tr('프론트엔드'), ['React 19 · Vite 6', 'Tailwind CSS v4', 'Recharts · motion', tr('초기 번들 412 KB (gzip)')], C.accent],
    [tr('AI · 데이터'), ['Gemini (Supervisor)', 'YOLOv8 · PatchCore', tr('GBR 잔여수명 예측'), 'Kafka · TimescaleDB · Neo4j'], C.cyan],
    [tr('보안'), [tr('키는 Cloudflare Worker에 보관'), tr('번들에 자격증명 없음'), tr('데모 모드 기본 동작'), tr('RBAC 역할 기반 접근')], C.warn],
    [tr('배포'), [tr('GitHub Actions 자동 배포'), tr('배포 후 브라우저 검증'), tr('커밋 해시 대조 확인'), tr('일 1회 상태 점검')], C.violet],
  ];
  ...
  s.addText(tr('데모 모드'), { x: M + 0.42, y: 5.24, w: 2, h: 0.3, margin: 0, fontFace: F.ko, fontSize: 12, bold: true, color: C.accent, isTextBox: true });
  s.addText(tr('API 키가 없어도 플랫폼 전체가 동작합니다. 챗봇과 추천 엔진은 내장된 공장 시뮬레이션에서 답을 만들고, AI 출력에는 “데모” 배지가 붙습니다.\n네트워크가 끊긴 전시장에서도 시연이 멈추지 않습니다. 실제 모델 연결이 필요하면 Worker 프록시를 켜기만 하면 됩니다.'), {
...
/* ───────────────────────── 14. 마무리 ───────────────────────── */
{
  ...
  s.addText(tr('요약'), { x: M, y: 1.2, w: COL, h: 0.3, margin: 0, fontFace: F.latin, fontSize: 11, bold: true, color: C.accent, charSpacing: 2, isTextBox: true });
  s.addText(tr('보여주는 대시보드가 아니라,\n결론을 내는 플랫폼입니다.'), {
    x: M, y: 1.62, w: 11.2, h: 1.5, margin: 0, lineSpacing: 46,
    fontFace: F.ko, fontSize: 34, bold: true, color: C.text, isTextBox: true });

  const closing = [
    [tr('협의'), tr('에이전트끼리 반대하고 조율하는 과정을 그대로 보여줍니다.')],
    [tr('승인'), tr('중요한 제어는 사람 손을 거쳐야만 나갑니다.')],
    [tr('근거'), tr('모든 판단이 이력으로 남아 사후에 검증됩니다.')],
  ];
  ...
  s.addText(tr('日本語 · English · 한국어  |  다크 / 라이트 테마  |  데모 모드 지원'), { x: M, y: 5.94, w: COL, h: 0.3, margin: 0, fontFace: F.ko, fontSize: 11, color: C.muted, isTextBox: true });
  foot(s, n, FOOT);
}

// fileURLToPath, not .pathname — the file names are not ASCII.
const out = fileURLToPath(new URL(`../${introFile}.pptx`, import.meta.url));
